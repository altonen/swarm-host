...
use crate::backend::substrate::network::{
    ...
    MAX_CONNECTIONS_ESTABLISHED_INCOMING,
    config::NetworkConfiguration,
	behaviour::{self, Behaviour, BehaviourOut},
	discovery::DiscoveryConfig,
	protocol::{self, Protocol, NotificationsSink},
	request_responses::{self, InboundFailure, RegisterError, OutboundFailure, RequestId, ResponseFailure},
	transport,
};
	...
	pub fn new(
		...
		let (mut swarm, _bandwidth): (Swarm<Behaviour>, _) = {
			...
			let (transport, bandwidth) = {
				...
				tracing::info!(target: "sub-libp2p", "local peer id: {local_peer_id}");
			...
			let behaviour = {
				...
				match result {
					Ok(b) => b,
					Err(RegisterError::DuplicateProtocol(proto)) =>
			...
			let builder = builder
				.connection_limits(
					ConnectionLimits::default()
						.with_max_established_per_peer(Some(MAX_CONNECTIONS_PER_PEER as u32))
	...
	async fn on_swarm_event(&mut self, event: BehaviourOut) {
    	match event {
			...
			BehaviourOut::NotificationStreamOpened {
				...
				tracing::info!("notification stream opened: {protocol}");
			...
			BehaviourOut::NotificationsReceived { remote, messages } => {
				tracing::trace!(target: "sub-libp2p", "notification received");
